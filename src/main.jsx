...
function App() {
  ...
  function saveNotice() {
    ...
    setCases((current) => current.map((item) => item.id === selected.id ? { ...item, status: 'NotificaciÃ³n guardada', note: notice.trim(), updated: today, step: Math.max(item.step, 2) } : item));
    setNotice('');
  }

  async function copyReceipt() {
    if (!selected) return;
    await navigator.clipboard.writeText(selected.receipt);
    setCopied(true);
      ...
      <section className="dashboard" id="casos">
        ...
        <article className="case-detail">
          {selected ? <>
            ...
            <div className="official-box"><div><ShieldCheck size={22}/><span><b>Consulta oficial de USCIS</b><small>Copia el recibo, abre USCIS y pega allÃ­ el nÃºmero.</small></span></div><div className="official-actions"><button className="copy-button" onClick={copyReceipt}>{copied ? 'Â¡Copiado!' : 'Copiar recibo'}</button><a href={officialUrl} target="_blank" rel="noreferrer">Abrir USCIS <ArrowUpRight size={16}/></a></div></div>
            <div className="notice-editor"><label htmlFor="latest-notice">Ãšltima notificaciÃ³n o nota familiar</label><textarea id="latest-notice" value={notice} onChange={(event) => setNotice(event.target.value)} placeholder="Copia aquÃ­ la notificaciÃ³n que aparece en USCIS o escribe un recordatorioâ€¦"/><button className="primary" onClick={saveNotice} disabled={!notice.trim()}><Check size={17}/> Guardar en el expediente</button></div>
